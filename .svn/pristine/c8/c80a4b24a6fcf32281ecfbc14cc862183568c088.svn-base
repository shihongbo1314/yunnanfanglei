                ...
                <el-timeline>
                    ...
                    >
                        ...
                        >
                            <div class="card_bot">
                                <div class="left">
                                    <p> <span> 项目名称： </span>{{ item.quotationInfo.number || '-' }} </p>
                                    <p
                                        v-if="item.fileName"
                                        @click.stop="curDetail(item.fileName)"
                                        style="cursor: pointer;"
                                    > <span> 报价单： </span> <span style="color:#2788EE">{{ item.fileName || '-' }}</span> </p>
                            ...
                            >
                                <div class="card_text">
                                    <span> {{ item.reviewInfo!=null ?  item.reviewInfo.userName : item.submitterInfo.userName }}</span>

                                </div>
                                <div class="card_filter">
                                    <svg-icon
                                        icon-class="tuihui"
                                        v-if="['18','19','20'].includes(item.state)"
                                    />
                                    <svg-icon
                                        icon-class="bianji"
                                        v-if="['22'].includes(item.state)"
                                    />
                                    <svg-icon
                                        icon-class="weitongguo"
                                        v-if="['8','11','17',].includes(item.state)"
                                    />
                                    <svg-icon
                                        icon-class="tixing"
                                        v-if="['9','15','21','23','24'].includes(item.state)"
                                    />
                                    <svg-icon
                                        icon-class="tongguo"
                                        v-if="['4','5','6','7','10','26'].includes(item.state)"
                                    />
                                    {{ item.state | stateFilterTwo}}
                                </div>
                                <div class="card_time">
