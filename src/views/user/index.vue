<template>
    <div class="app-container" style="height:100%">
        ...
        <div style="background:#FFF;height: calc(100% - 88px);">
            <el-table v-loading="listLoading" :data="list" element-loading-text="Loading" border fit
                highlight-current-row height="70vh">
                ...
                <el-table-column label="区划" align="center">
                    <template slot-scope="scope">
                ...
                <el-table-column label="角色" align="center">
                    <template slot-scope="scope">
                        <span>{{ scope.row.roleIdMap.name }}</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="用户资格证号" align="center">
                    <template slot-scope="scope">
                ...
                <el-table-column label="二级角色" align="center">
                    <template slot-scope="scope">
                        <span v-for="it, is in scope.row.userMultiRoleList">{{ it.roleIdMap.name + ',' }}</span>
                    </template>
                </el-table-column>
                <el-table-column align="center" prop="created_at" label="编辑" width="300"
                    v-if="sessionlevel != '3' && sessionName == '管理员'">
                    <template slot-scope="scope">
                        <el-button size="mini" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
                        <el-button size="mini" type="primary" @click="handleEdittwo(scope.row)">二级角色设置</el-button>
        ...
        <!-- 添加弹框 -->
        <el-dialog width="30%" title="添加用户" :visible.sync="dialog.innerVisible" append-to-body @close='close'>
            <el-form :label-position="labelPosition" label-width="100px" :model="formLabelAlign" ref="ruleForm">
                ...
                <el-form-item label="用户资格证号" prop="qualifications">
                ...
                <el-form-item label="行政区域">
                    <el-radio-group v-model="formLabelAlign.radio" @change="changeBox">
                        <el-radio :label="3">县级</el-radio>
                        <el-radio :label="6">市级</el-radio>
                        <el-radio :label="9" v-if="sessionlevel == '1'">省级</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="市级" prop="regionId" v-if="formLabelAlign.radio == 6">
                    <el-select v-model="formLabelAlign.regionId" clearable placeholder="行政区域">
                        <el-option v-for="item in regionIdlevelList" :key="item.id" :label="item.name" :value="item.id">
                            <span>{{ item.name }}</span>
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="省级" prop="regionId" v-if="formLabelAlign.radio == 9">
                    <el-select v-model="formLabelAlign.regionId" clearable placeholder="行政区域">
                        <el-option v-for="item in regionIdlevelListsheng" :key="item.id" :label="item.name"
                            :value="item.id">
                            <span>{{ item.name }}</span>
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="市级" prop="regionId" v-if="formLabelAlign.radio == 3 && sessionlevel == '1'">
                    <el-select v-model="formLabelAlign.flag" clearable placeholder="行政区域" @change="changeShi">
                        <el-option v-for="item in regionIdlevelxian" :key="item.code" :label="item.name"
                            :value="item.code">
                            <span>{{ item.name }}</span>
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="县级" prop="regionId" v-if="formLabelAlign.radio == 3">
                    <el-select v-model="formLabelAlign.regionId" clearable placeholder="行政区域">
                        <el-option v-for="item in regionIdlevelListxian" :key="item.id" :label="item.name"
                            :value="item.id">
                            <span>{{ item.name }}</span>
                        </el-option>
                    </el-select>
                ...
                <el-form-item label="角色" prop="roleId">
                    <el-select v-model="formLabelAlign.roleId" clearable placeholder="角色编号">
                        <el-option v-for="item in regionIdList" :key="item.id" :label="item.name" :value="item.id">
                            <span>{{ item.name }}</span>
                        </el-option>
                    </el-select>
                </el-form-item>
    ...
    justify-content: flex-end;">
        ...
        <!-- 编辑弹框 -->
        <el-dialog width="30%" title="编辑用户" :visible.sync="dialogone.innerVisible" append-to-body>
            <el-form :label-position="labelPosition" label-width="100px" :model="formLabel" ref="ruleForm">
                ...
                <el-form-item label="角色" prop="roleId">
                    <el-select v-model="formLabel.roleId" clearable placeholder="角色编号">
                        <el-option v-for="item in regionIdList" :key="item.id" :label="item.name" :value="item.id">
                            <span>{{ item.name }}</span>
                        </el-option>
                    </el-select>
                ...
                <el-form-item label="统计状态" prop="statisticalStatus">
                    <el-tooltip :content="formLabel.statisticalStatus" placement="top">
                        <el-switch v-model="formLabel.statisticalStatus" active-color="#ff4949" inactive-color="#13ce66"
                            active-value="不统计" inactive-value="统计">
                        </el-switch>
                    </el-tooltip>
                </el-form-item>
                <el-form-item style="display: flex;justify-content: flex-end;">
                    <el-button type="primary" @click="compile()">保存</el-button>
                    <el-button @click="dialogone.innerVisible = false">取消</el-button>
                </el-form-item>
            </el-form>
        </el-dialog>
        <!-- 二级角色设置 -->
        <el-dialog width="30%" title="二级角色设置" :visible.sync="dialogtwo.innerVisible" append-to-body>
            <el-form :label-position="labelPosition" label-width="100px" :model="formLabel" ref="ruleForm">

                <el-form-item label="角色编号" prop="roleId">
                    <el-select v-model="formLabeltwo" clearable multiple placeholder="角色编号">
                        <el-option v-for="item in regionIdList" :key="item.id" :label="item.name" :value="item.id">
                            <span>{{ item.name }}</span>
                        </el-option>
                    </el-select>
                ...
                <el-form-item style="display: flex;
    justify-content: flex-end;">
                    <el-button type="primary" @click="compiletwo()">保存</el-button>
...
export default {
    ...
    methods: {
        getcompany() {
            ...
            this.formInline.company = params.company;
        },
        fetchData() {
        ...
        handleEdit(index, row) {
            ...
            this.formLabel.statisticalStatus =
                row.statisticalStatus == "1" ? "统计" : "不统计";
            ...
            GetroleList({
                ...
                res.data.records.map((item) => {
                    if (this.roleIdMapName == item.name) {
                        ...
                        if (params.regionIdMap.level == "2") {
                            this.regionIdList = this.regionIdList.filter(item => item.name != "审核人");
                        }
                    } else {
                        this.regionIdList.push(item);
